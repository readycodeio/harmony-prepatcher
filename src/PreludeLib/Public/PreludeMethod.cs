...
public class PreludeMethod
{
	...
	{
		return [.. AccessTools.GetFieldNames(typeof(PreludeMethod))];
	}
	
	public PreludeMethod()
	{
		// empty
	}
	
	public PreludeMethod(ModuleDefinition module, HarmonyMethod harmonyMethod)
	{
	...
	public static PreludeMethod Merge(List<PreludeMethod>? attributes)
	{
		...
		attributes.ForEach(attribute =>
		{
			...
			HarmonyFields().ForEach(f =>
			{
				...
				if (val is not null && (f != nameof(Priority) || (int)val != -1))
					PreludeMethodExtensions.SetValue(resultTrv, f, val);
			});
		});
		return result;
	}
