...
{
    private class PatchEntry(string? category, HarmonyMethod patchInfo, Delegate del, EventInfo ev, OriginalMethodEntry owner)
    {
    ...
    private void EnsurePatchOwned(MethodBase original, HarmonyMethod patchInfo, out PatchEntry patchEntry)
    {
        if (!IsOwnedPatch(original, patchInfo.method, out var p))
            throw new ArgumentException("The specified patch was not applied.", nameof(patchInfo));
